...
export default function AdminDashboard() {
  ...
  return (
    <div className="h-[100dvh] p-8 max-w-6xl mx-auto flex flex-col relative overflow-x-hidden justify-between">
      ...
      <header className="flex justify-between items-center pb-8 border-b border-white/5 relative z-20">
        <div className="flex items-center gap-6">
          <GlitchLogo size="xs" isStatic={true} />

          <div>
            <h1 className="text-header-premium text-3xl">
              CIL LENGUAS
            </h1>
            <p className="text-[9px] font-jetbrains text-purple-400 uppercase tracking-[0.5em] mt-2 opacity-60 flex items-center gap-2">
              <ShieldCheck className="w-3 h-3 text-whapigen-cyan animate-pulse" /> COMMAND CENTER
            </p>
          </div>
        </div>
        <button
          onClick={handleLogout}
          className="flex items-center gap-3 text-whapigen-red text-[10px] font-jetbrains hover:text-white hover:bg-whapigen-red transition-all rounded-full px-8 py-3 border border-whapigen-red/20 font-black uppercase tracking-widest shadow-lg hover:shadow-neon-pulse-red"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10 flex-grow py-12 items-start">
        ...
        <div className="md:col-span-2 bg-black/40 backdrop-blur-xl border border-white/5 rounded-[40px] p-12 flex flex-col gap-10 shadow-2xl relative overflow-x-hidden group/missions animate-in slide-in-from-right duration-700">
          ...
          {loading ? (
            ...
            <div className="grid gap-6 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
              {rooms.map((room) => (
                <div key={room.id} className="flex justify-between items-center p-8 bg-white/5 border border-white/5 hover:border-purple-500/30 hover:shadow-[0_0_40px_rgba(147,51,234,0.1)] transition-all group/item rounded-[30px] backdrop-blur-2xl relative overflow-x-hidden">
